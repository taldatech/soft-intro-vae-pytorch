...
def load_dataset(class_name='TFRecordDataset', data_dir=None, verbose=False, **kwargs):
...
def create_from_images_ffhq(tfrecord_dir, image_dir, shuffle):
    import functools
    import operator
    print('Loading images from "%s"' % image_dir)
    # image_filenames = sorted(glob.glob(os.path.join(image_dir, '*')))
    image_filenames = functools.reduce(operator.iconcat, [glob.glob(d + '/*.png') for d in glob.glob(image_dir + '/*')],
                                       [])
    if len(image_filenames) == 0:
        error('No input images found')
...
def execute_cmdline(argv):
    ...
    p = add_command('create_from_images_ffhq', 'Create dataset from a directory full of images.',
                    'create_from_images datasets/mydataset myimagedir')
    p.add_argument('tfrecord_dir', help='New dataset directory to be created')
    p.add_argument('image_dir', help='Directory containing the images')
    p.add_argument('--shuffle', help='Randomize image order (default: 1)', type=int, default=1)
